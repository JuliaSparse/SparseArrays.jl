...
@testset "Concatenation" begin
    let m = 80, n = 100
        ...
        Vr = vec(Hr)
        @test Array(V) == Vr
        Vnum = @inferred vcat(A..., zero(Float64))
        Vnum2 = @inferred sparse_vcat(map(Array, A)..., zero(Float64))
        ...
        Vnum = @inferred vcat(zero(Float64), A...)
        Vnum2 = @inferred sparse_vcat(zero(Float64), map(Array, A)...)
        @test Vnum isa SparseVector{Float64,Int}
        @test Vnum2 isa SparseVector{Float64,Int}
        @test length(Vnum) == length(Vnum2) == m*n + 1
        ...
        x = sparsevec([1], [3.0], 1)
        X = @inferred hvcat((2, 2), 3.0, x, 3.0, x)
        @test issparse(X)
    end
