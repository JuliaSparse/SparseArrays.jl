...
using ..SparseArrays: FactorLock, @_readlock, @_writelock
...
Every call on `F` holds a readers-writer lock internal to `F`, so one `F` can be shared
between tasks. Solves and queries run in parallel. `ldiv!(x, F, b)` reuses a solve
workspace kept in `F`, so it runs alongside them but one `ldiv!` at a time on each `F`.
The calls that change `F` (`cholesky!`, `ldlt!`, the low-rank updates and `free!`) wait
for the calls in progress and run alone; a steady stream of them can hold solves back.
...
mutable struct Factor{Tv<:VTypes, Ti<:ITypes} <: Factorization{Tv}
    ...
    # Every call on the factor holds `_lock`: reads share it, and refactorizations,
    # updates and `free!` take it exclusively. `ldiv!` also holds `_wslock` for the buffers
    # above, always after `_lock`.
    const _lock::FactorLock
    const _wslock::ReentrantLock
...
_factorlock(F::Factor) = getfield(F, :_lock)
...
mutable struct FactorComponent{Tv, S, Ti} <: AbstractMatrix{Tv}
  ...
  function FactorComponent{Tv,S,Ti}(F::Factor{Tv,Ti}) where {Tv,S,Ti}
      s = @_readlock F unsafe_load(pointer(F))
      is_ll = (s.is_ll != 0)
...
@eval begin
    ...
    function check_factor(F::Factor{Tv, $TI}) where Tv<:VTypes
        @_readlock F (@checked $(cholname(:check_factor, TI))(F, getcommon($TI))) != 0
    end
    function rcond(F::Factor{Tv, $TI}) where Tv<:VTypes
        @_readlock F @checked $(cholname(:rcond, TI))(F, getcommon($TI))
    end
    ...
    function copy(F::Factor{Tv, $TI}) where Tv<:VTypes
        c = @_readlock F @checked $(cholname(:copy_factor, TI))(F, getcommon($TI))
        Factor{Tv, $TI}(c)
    end
    ...
    function factorize!(A::Sparse{Tv, $TI}, F::Factor{Tv, $TI}) where Tv<:VTypes
        @_writelock F @checked $(cholname(:factorize, TI))(A, F, getcommon($TI))
        return F
    end
    function factorize_p!(A::Sparse{Tv, $TI}, β::Real, F::Factor{Tv, $TI}) where Tv<:VTypes
        ...
        @_writelock F @checked $(cholname(:factorize_p, TI))(A, Float64[β, 0], C_NULL, 0, F, getcommon($TI))
        return F
    end
...
function get_perm(F::Factor)
    ...
    return @_readlock F GC.@preserve F begin
...
free!(F::Factor) = @_writelock F _free_factor!(F)
# The finalizer, which must not lock.
...
function size(F::Factor, i::Integer)
    ...
    n = @_readlock F _size(F)
    return i <= 2 ? n : 1
end
size(F::Factor) = (n = @_readlock F _size(F); (n, n))
...
for (T, f) in ((:Dense, :solve), (:Sparse, :spsolve))
    @eval begin
        ...
        function (\)(L::FactorComponent{T,:PtL}, B::$T) where T
            F = Factor(L)
            @_readlock F ($f)(CHOLMOD_L, F, ($f)(CHOLMOD_P, F, B)) # Confusingly, CHOLMOD_P solves P'x = b
        end
        function (\)(L::FactorComponent{T,:UP}, B::$T) where T
            F = Factor(L)
            @_readlock F ($f)(CHOLMOD_Pt, F, ($f)(CHOLMOD_Lt, F, B))
        end
        ...
        function (\)(L::FactorComponent{T,:PtLD}, B::$T) where T
            F = Factor(L)
            @_readlock F ($f)(CHOLMOD_LD, F, ($f)(CHOLMOD_P, F, B))
        end
        function (\)(L::FactorComponent{T,:DUP}, B::$T) where T
            F = Factor(L)
            @_readlock F ($f)(CHOLMOD_Pt, F, ($f)(CHOLMOD_DLt, F, B))
        end
    end
end
...
for TI in IndexTypes
    @eval solve!(x::StridedVecOrMat{T}, L::Factor{T, $TI}, b::StridedVecOrMat{T}) where {T<:VTypes} =
        @_readlock L @lock getfield(L, :_wslock) _solve!(x, L, b)

    # The caller holds the read lock of `L` and its `_wslock`.
    ...
    @eval function ldiv!(x::StridedVecOrMat{T},
        ...
        # Solves share the read lock of `L`; `_wslock` serializes the use of its buffers.
        @_readlock L begin
            n = _size(L)
            if n != size(b, 1)
            ...
            @lock getfield(L, :_wslock) _solve!(x, L, b)
        end
        return x
    end
end
...
diag(F::Factor) = @_readlock F _diag(F)
...
function LinearAlgebra.logabsdet(F::Factor{Tv}) where Tv<:VTypes
    is_ll, dg = @_readlock F (unsafe_load(pointer(F)).is_ll != 0, _diag(F))
    res = zero(real(Tv))
    sgn = one(real(Tv))
    if is_ll
...
issuccess(F::Factor) = @_readlock F _issuccess(F)
